...
| --- | --- | --- |
| 2026-08-25 | **El `apply` de migraciones corría con el timeout interactivo de 15 s** (`T-260824-lz-apply-timeout-bulk`, subtarea [`86e2z73bf`](https://app.clickup.com/t/86e2z73bf)) — `_prepared` toma una perilla `bulk` que piden `apply` y `rollback_to`; `reconcile_partial` la pasa en su propia conexión. `stamp` y `get_current_version` se quedan con el interactivo a propósito: no ejecutan SQL del usuario. **El eje es la OPERACIÓN, no la versión** — lo contrario de la hipótesis que este mismo ítem traía anotada, y por un motivo concreto: cuánto tarda un `ALTER TABLE` depende del volumen de datos de la BD DESTINO, no del texto de la sentencia, así que una marca por versión codificaría una propiedad que la versión no tiene. Y los 15 s no eran una protección que se pierda: en MySQL son `read_timeout`/`write_timeout` de socket DEL CLIENTE y no cancelan nada en el motor — rompían la conexión mientras el servidor seguía ejecutando, el checkpoint no registraba la sentencia, el motor la completaba igual y el próximo `apply` la reejecutaba: un **bucle** con la contabilidad desincronizada del plano físico | 3 casos nuevos en `test_migration_runner.py` que interceptan `database_connection` y afirman sobre el FLAG (uno de ellos fija que `stamp` NO lo tome, que es lo que hace que el eje sea la operación), y sin regresión en 8 suites vecinas (192 checks). **Sin migración Alembic.** **Nada verificado contra motores reales**: en particular que un `ALTER TABLE` largo termine de verdad sin que el cliente corte. Consecuencia operativa aceptada y documentada: `POST /migrations/apply` es síncrono, así que un apply largo sostiene la request y su hilo |
