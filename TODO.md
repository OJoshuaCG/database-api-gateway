...
| T-260824-lz-apply-timeout-bulk | El `apply` de migraciones sigue con el timeout interactivo de 15 s | `execute_adhoc` ya tomó la perilla `bulk` (la usa la conversión de collation), pero **`_apply_one`/`_prepared`** —el camino de Alembic, que es el que aplica una migración de blueprint— siguen con `database_connection(target, db_name)` sin `bulk` ⇒ `REMOTE_STATEMENT_TIMEOUT_MS=15000`. En MySQL/MariaDB eso es `read_timeout`/`write_timeout` **de socket del CLIENTE**: no cancela nada en el motor. Una migración con DDL pesado (un `ALTER TABLE` sobre una tabla grande) rompe la conexión a los 15 s, el checkpoint no registra la sentencia, **el motor la completa igual**, y el próximo `apply` la reejecuta ⇒ bucle. No se tocó en la misma entrega porque subir el timeout a TODAS las migraciones es un cambio de comportamiento global (una migración descontrolada pasaría de fallar en 15 s a colgarse una hora) y merece su propia decisión: probablemente una perilla por versión, no un default nuevo. | — |
