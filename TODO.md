...
| T-260825-lz-arnes-no-equivale-a-pytest | El ejecutor directo del scratchpad NO es equivalente a `pytest`, y enmascara defectos | El arnés construye un `TestClient` para **cada** test sin mirar qué fixtures declara, así que corre `Base.metadata.create_all` como efecto secundario. Consecuencia: un test que dependa del esquema del gateway **sin pedirlo** pasa en el arnés y falla bajo `pytest`. Detectado en carne propia: dos tests de `T-260824-lz-apply-timeout-bulk` daban 19/19 en el arnés y fallaban sin esquema (`no such table: migration_statement_progress`), porque `_write_revision_files` consulta `migration_progress.get_progress`. Los otros 17 del archivo son autocontenidos, así que la convención existía y se rompió sin que nada avisara. **No es el primer caso de esta clase**: ya había pasado con `@pytest.mark.parametrize`, que el arnés ignoraba y dejaba 3 casos de `is_capturable` sin correr en ninguna verificación previa. Mientras la política sea no correr `pytest`, el arnés tiene que **inyectar solo las fixtures declaradas** (y fallar ruidosamente ante una que no soporte) en vez de dar de más; y conviene una pasada que corra cada archivo de test SIN el cliente para detectar dependencias no declaradas. | — |
